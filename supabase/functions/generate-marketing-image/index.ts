...
`;
    ...
    const layouts = [
      hasReference
        ? `Split layout: left 25% is a sophisticated dark panel with the headline stacked vertically, right 75% features the reference photo scaled to fit ENTIRELY within this area — the COMPLETE photo must be visible including all hair and heads, even if it means leaving some background padding around the photo. Never zoom in or crop any edge. Thin gold border around the entire image. Decorative dotted line divider between text and photo.`
      ...
      `Full-bleed cinematic photo as background with a subtle dark gradient overlay (40% opacity). Headline centered in bold uppercase.${hasBrandName ? ' Brand name at top in smaller text.' : ''} Thin decorative line separators above and below the headline. Scale the reference photo to fit entirely within the frame — never zoom in or crop edges. If the reference photo shows a before-and-after transformation, you MUST display the FULL photo without cropping either side.`,
      `Framed composition: sophisticated dark background with a centered rectangular photo inset (white or gold thin border around the photo). Headline ABOVE the photo in large bold text.${hasBrandName ? ' Brand name and tagline BELOW the photo.' : ''} Clean, editorial layout. Scale the reference photo to fit entirely within the frame — never zoom in or crop edges. If the reference photo shows a before-and-after transformation, you MUST display the FULL photo without cropping either side.`,
    ];
