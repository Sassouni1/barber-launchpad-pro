...
`;
    ...
    const layouts = [
      hasReference
        ? `IMPORTANT: If the reference photo is a before-and-after (two sides), override the layout below — center the photo at a size that shows both sides fully, then place text above or below. Split layout: left 25% is a sophisticated dark panel with the headline stacked vertically in bold white and gold alternating words. Right 75% is the reference photo — if it is a before-and-after photo, scale it down enough to show BOTH sides completely with no cropping on any edge. Otherwise, fill most of the right side as the HERO element. Thin gold border around the entire image. Decorative gold dotted-line divider between the text panel and photo. REMINDER: The person in this photo is REAL — use their exact pixels. Do NOT generate a new person.`
      ...
      hasReference
        ? `IMPORTANT: If the reference photo is a before-and-after (two sides), override the layout below — center the photo at a size that shows both sides fully, then place text above or below. Reference photo as large full-bleed background filling the canvas. Headline in bold uppercase positioned in the upper-left or upper area with a subtle dark gradient behind the text for readability (max 40% opacity).${hasBrandName ? ' Brand name + CTA at bottom.' : ''} Thin gold outer frame border wrapping the entire composition. The photo is the HERO — it should dominate the image. REMINDER: The person in this photo is REAL — use their exact pixels. Do NOT generate a new person.`
      ...
      hasReference
        ? `IMPORTANT: If the reference photo is a before-and-after (two sides), override the layout below — center the photo at a size that shows both sides fully, then place text above or below. Dark background with the reference photo as a large centered element with a thin white or gold border around just the photo. Headline ABOVE the photo in large bold white and gold text.${hasBrandName ? ' Brand name and tagline BELOW the photo.' : ''} Clean, editorial layout. The photo should be prominent and large — the focal point of the composition. REMINDER: The person in this photo is REAL — use their exact pixels. Do NOT generate a new person.`
