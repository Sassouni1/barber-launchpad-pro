...
`;
    ...
    const layouts = [
      hasReference
        ? `Split layout: left 25% is a sophisticated dark panel with the headline stacked vertically, right 75% is a dark background area that CONTAINS the reference photo. The photo must be SMALLER than this area — shrink it to about 85% of the available space so there is visible dark padding on ALL four sides (top, right, bottom, left). The entire photo including every person's full head and hair must be visible with clear margins. NEVER let the photo touch any edge. Thin gold border around the entire image. Decorative dotted line divider between text and photo.`
      ...
      `Dark background with the reference photo centered and INSET — the photo should occupy about 80% of the canvas so there is visible dark padding on all sides. Headline centered in bold uppercase ABOVE the photo.${hasBrandName ? ' Brand name at top in smaller text.' : ''} Thin decorative line separators. Subtle dark gradient overlay (max 30% opacity) on the photo for text readability. The photo must NOT touch any edge of the canvas. If it's a before-and-after, BOTH sides must be fully visible.`,
      `Framed composition: sophisticated dark background with the reference photo centered as a rectangular inset at about 75% of the canvas size — there MUST be visible dark padding on all four sides between the photo and the image edges. White or gold thin border around the photo. Headline ABOVE the photo in large bold text.${hasBrandName ? ' Brand name and tagline BELOW the photo.' : ''} Clean, editorial layout. The photo must NEVER touch or bleed to any edge.`,
    ];
