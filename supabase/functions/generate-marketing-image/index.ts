...
`;
    ...
    const layouts = [
      ...
      hasReference
        ? `Dark background with the reference photo placed as a large centered element without any modifications, preserving original pixels, with a thin white or gold border around just the photo. Scale it so the entire photo is visible (including both sides if it is a before-and-after). If the photo contains two people (before-and-after), scale it so that both heads occupy no more than 70% of the available panel height, leaving at least 15% padding above the tallest head. The canvas background should use the prescribed tone from Design Rule 2. Feather the photo's edges into this canvas tone so there is no harsh boundary. Headline ABOVE the photo in large bold white and gold text.${hasBrandName ? ' Brand name and tagline BELOW the photo.' : ''} Clean, editorial layout. The photo should be prominent and large — the focal point of the composition.`
