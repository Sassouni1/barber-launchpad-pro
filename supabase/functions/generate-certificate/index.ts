...
function normalizeShippingAddress(value: unknown) {
  ...
  const missing = Object.entries(normalized)
    .filter(([key, fieldValue]) => key !== 'addressLine2' && !fieldValue)
    .map(([key]) => key);
  if (missing.length > 0) {
...
function resolveCertificateLayout(layout: Record<string, unknown>, width: number, height: number) {
  ...
  const nameCoordinatesAreVisible =
    isCoordinateVisible(storedNameX, width) &&
    isCoordinateVisible(storedNameY, height);
  const dateCoordinatesAreVisible =
    isCoordinateVisible(storedDateX, width) &&
    isCoordinateVisible(storedDateY, height);

  const nameX = nameCoordinatesAreVisible ? storedNameX : Math.round(width * FALLBACK_LAYOUT_RATIOS.nameX);
  const nameY = nameCoordinatesAreVisible ? storedNameY : Math.round(height * FALLBACK_LAYOUT_RATIOS.nameY);
  const dateX = dateCoordinatesAreVisible ? storedDateX : Math.round(width * FALLBACK_LAYOUT_RATIOS.dateX);
  const dateY = dateCoordinatesAreVisible ? storedDateY : Math.round(height * FALLBACK_LAYOUT_RATIOS.dateY);
  ...
  return {
    ...
    dateX,
    dateY,
    usedFallback: !nameCoordinatesAreVisible || !dateCoordinatesAreVisible || nameMaxWidth !== storedNameMaxWidth,
    stored: {
      nameX: storedNameX,
      nameY: storedNameY,
      nameMaxWidth: storedNameMaxWidth,
      dateX: storedDateX,
      dateY: storedDateY,
    },
  };
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Generating certificate for:', {
      ...
      courseId,
      certificateName,
      hasShippingAddress: !!normalizedShippingAddress,
      debug,
    });
    ...
    console.log('Using pixel coordinates:', {
      ...
      dateX,
      dateY,
      usedFallback: resolvedLayout.usedFallback,
      stored: resolvedLayout.stored,
    ...
    if (normalizedShippingAddress) {
      ...
      if (fulfillmentError) {
        ...
        throw new Error(`Failed to save fulfillment request: ${fulfillmentError.message}`);
      }

      fulfillmentRequest = fulfillmentData;
      console.log('Fulfillment request saved:', fulfillmentRequest);
    }
