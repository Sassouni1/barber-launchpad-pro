...
async function getAccessToken(supabase: SupabaseClient): Promise<{ accessToken: string; locationId: string }> {
  ...
  if (!encryptionKey) {
    throw new Error("GHL is not configured. Set GHL_ACCESS_TOKEN/GHL_LOCATION_ID or connect OAuth with GHL_ENCRYPTION_KEY.");
  }
  ...
  if (error || !tokenRecord) {
    throw new Error("No GHL OAuth connection found");
  }
...
async function resolveChrisContactId(accessToken: string, locationId: string): Promise<string> {
  const configuredContactId = Deno.env.get("GHL_CHRIS_CONTACT_ID");
  ...
  if (searchRes.ok) {
    const data = await searchRes.json() as ContactSearchResponse;
  ...
  if (!createRes.ok) {
    throw new Error(`Failed to create/find Chris GHL contact: ${createRes.status} ${await createRes.text()}`);
  }

  const created = await createRes.json() as ContactCreateResponse;
  const contactId = created.contact?.id || created.id;
  if (!contactId) throw new Error("GHL create contact response did not include contact id");
  return contactId;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const studentEmail = profile?.email || "unknown email";
    ...
    if (!smsRes.ok) {
      throw new Error(`GHL SMS failed: ${smsRes.status} ${await smsRes.text()}`);
    }
