...
function HairColorPicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <div className="space-y-3"><div className="flex items-center justify-between gap-3"><Label>Hair color</Label><span className="text-xs text-muted-foreground">{value ? `Selected: ${value}` : "Select a shade"}</span></div><div className="gap-3" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))" }}>{hairColorOptions.map(([code]) => <button key={code} type="button" onClick={() => onChange(code)} aria-label={`Select hair color ${code}`} className={`relative min-w-0 overflow-hidden rounded-lg border bg-white transition-all ${value === code ? "border-primary ring-2 ring-primary ring-offset-2 ring-offset-card" : "border-border hover:border-primary/60"}`} style={{ aspectRatio: "1 / 1" }}><img src={`/hair-color-tiles/${code.slice(1)}.png?v=4`} alt="" className="h-full w-full object-contain" /><span className="absolute inset-x-0 bottom-0 bg-white py-1 text-[11px] font-bold leading-none text-slate-900">{code}</span></button>)}</div><p className="text-xs text-muted-foreground">Choose the numbered shade that best matches the hair color guide.</p></div>;
}
